  ...
  return createSystem({
    ...
    function compareViewAndModel(flask) {
        ...
        flask.stack.forEach(modelType => {
            ...
            if (!peak || peak.type !== modelType)
                modelGroups.push({ type: modelType, volume: 1, state: LiquidLayerState.Pouring })
            else peak.volume++
        })
    }
    ...
    function placeSprites(layers, flaskVolume) {
        ...
        layers.forEach((id) => {
            ...
            const { state, volume } = layer
            const copy = layerEntity[VC.Copy]
            if (state === LiquidLayerState.Pouring) {
                const targetHeight = minLayerHeight * volume
                let nextHeight = copy.height
                if (!isCloseEnough(targetHeight, nextHeight)) {
            ...
            if (volume === 0 && isCloseEnough(copy.height, 0)) {
                w.addComponent(layerEntity, VC.Remove)
            }
        })
    }
    ...
    function removeLayer(layer) {
        const layerData = layer[VC.LiquidLayer]
        layerData.state = LiquidLayerState.Pouring
        layerData.volume = 0
        newLayers.push(layer.id)
    }
    ...
    return createSystem({
        ...
        onCleanUp() {
            ...
            for (const entity of queries.remove) {
                if (entity[VC.Copy]) entity[VC.Copy].kill = true
            }

            for (const entity of queries.copy) {
    ...
    var BoardViewEvent; (function (BoardViewEvent) {
        ...
        const FADED_OUT = 'FADED_OUT'; BoardViewEvent["FADED_OUT"] = FADED_OUT;
    })(BoardViewEvent || (BoardViewEvent = {}));
    ...
    async function fadeFlasks() {
        ...
        for (const flaskView of G.view.w.queries.flask) {
            ...
            animations.push(mod(async () => {
                ...
                G.view.w.addComponent(flaskView, VC.IsFaded)
            }))
        }
  ...
  return createSystem({
    ...
    return {
        [LevelMode.CLASSIC_RANDOM]: {
            ...
            flasks: {
                full: utils.roundTo(random.range(5, 10), 2),
                empty: 2
            },
            maxColors: 10
        },
...
templates.templates["LiquidLayer"] = {
    name: "LiquidLayer",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
...
templates.templates["UI.StartGame"] = {
    name: "UI.StartGame",
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    visible: true,
...
var Queue = class {
  ...
  peek() {
    return this.count > 0 ? this.buffer[this.head] : void 0;
  }
  isEmpty() {
    return this.count === 0;
  }
  ...
  grow() {
    ...
    let i = 0;
    while (this.count > 0) {
      newBuffer[i++] = this.buffer[this.head];
      this.head = (this.head + 1) % this.buffer.length;
      this.count--;
    }
    this.head = 0;
...
var HFSM = class _HFSM {
  constructor(config, parent = null) {
    this.config = config;
    this.parent = parent;
    this.child = null;
    this.fsmCache = this.parent?.fsmCache ?? {};
    this.transitions = {};
    ...
    /** Алфавит событий */
    this.eventABC = /* @__PURE__ */ new Set();
    const { transitions } = this;
    Object.entries(config.states).forEach(([state, config2]) => {
      config2.transitions?.forEach((t) => {
        const events = transitions[state] ?? (transitions[state] = {});
        const transitionArray = events[t.event] ?? (events[t.event] = []);
  ...
  executeActions(actions) {
    ...
    for (const action of actions) {
      if (action.constructor.name === "AsyncFunction") this.executeAsyncAction(action);
      else this.executeSyncAction(action);
    }
  }
  ...
  handleActionResult(result) {
    ...
    this.eventQueue.enqueue(result);
  }
  executeOn(event, state) {
    const stateConfig = this.getStateConfig(state);
    if (!stateConfig) return;
    this.executeActions(stateConfig[event]);
  }
  findTransition(event) {
    const transitions = this.transitions?.[this.context.state]?.[event];
    if (!transitions) return null;
    const localTransition = transitions.find((t) => {
      if (t.condition) return t.condition(this.context);
      else return true;
    });
    if (localTransition) return localTransition;
    if (this.child) {
      const childResult = this.child.findTransition(event);
      if (childResult) {
        return childResult;
      }
    }
  ...
  transition(transition) {
    ...
    this.setTransitionData(from, to, transition.event);
    this.emit("before-transition" /* BEFORE_TRANSITION */);
    ...
    if (newStateConfig?.fsm) {
      if (!this.fsmCache[to]) this.fsmCache[to] = new _HFSM(newStateConfig.fsm, this);
  ...
  dispatch(event) {
    if (!this.eventABC.has(event)) return;
    if (!this.context) throw new ReferenceError("Context not found");
    const { eventQueue } = this;
    eventQueue.enqueue(event);
    while (!eventQueue.isEmpty()) {
      const e = eventQueue.dequeue();
      const transition = this.findTransition(e);
      if (transition) {
        this.transition(transition);
      }
    }
    this.emit("exit-state-machine" /* EXIT_STATE_MACHINE */);
  }
  getChild() {
    return this.child;
  }
  recordAllEvents() {
    const { eventABC } = this;
    const collectFromConfig = (config) => {
      Object.values(config.states).forEach((stateConfig) => {
        if (!stateConfig) return;
        const sc = stateConfig;
        sc.transitions?.forEach((t) => {
          eventABC.add(t.event);
        });
        if (sc.fsm) {
          collectFromConfig(sc.fsm);
        }
      });
    };
    collectFromConfig(this.config);
  }
  emit(event) {
    this.hooks.emit(event, this.transitionData);
  }
  setTransitionData(from, to, e) {
    const { transitionData } = this;
    transitionData.from = from;
    transitionData.event = e;
    transitionData.to = to;
    transitionData.context = this.context;
  }
};
var HOOKS = /* @__PURE__ */ ((HOOKS2) => {
  HOOKS2["BEFORE_TRANSITION"] = "before-transition";
  HOOKS2["AFTER_TRANSITION"] = "after-transition";
  HOOKS2["BEFORE_ENTER"] = "before-enter";
  HOOKS2["AFTER_ENTER"] = "after-enter";
  HOOKS2["BEFORE_EXIT"] = "before-exit";
  HOOKS2["AFTER_EXIT"] = "after-exit";
  HOOKS2["ENTER_STATE_MACHINE"] = "exit-state-machine";
  HOOKS2["EXIT_STATE_MACHINE"] = "exit-state-machine";
  return HOOKS2;
})(HOOKS || {});
...
const utils = mod(() => {
  return {
    bind(obj, ...methods) { methods.forEach(method => obj[method.name] = method.bind(obj)) },
    arrayOf(size, func) { return Array.from({ length: size }, func) },
    shuffle(array) {
    ...
    peak(arr) { return arr[arr.length - 1] },
    difference(a, b) { return Math.abs(a - b) },
    /*** Проверка погрешности */
    isClose(a, b, toleracne) { return toleracne >= Math.abs(a - b) },
    roundTo(value, factor) { return Math.round(value / factor) * factor },
    lerp(from, to, t, clamp = false) {
      let result = u.lerp(from, to, t)
      if (clamp) {
        let min = from, max = to
        if (min > max) min = to, max = from
...
class StateStack {
  __init() {this.stateStack = [this.state]}
  constructor( state) {;this.state = state;StateStack.prototype.__init.call(this); }
  pushState(state) { this.stateStack.push(state) }
  popState() { return this.stateStack.pop() }
  changeState(state) { this.popState(); this.pushState(state) }
}

class Switch {
   __init2() {this.map = new Map(this.cases)}
  constructor( cases) {;this.cases = cases;Switch.prototype.__init2.call(this); }
  case(key) {
    const fn = this.map.get(key)
    if (fn) fn()
    else {
      const fn = this.map.get('default')
      if (fn) fn()
    }
  }
};
...
class ECSEventEmitter {constructor() { ECSEventEmitter.prototype.__init.call(this);ECSEventEmitter.prototype.__init2.call(this);ECSEventEmitter.prototype.__init3.call(this); }
   __init() {this.entityHandlers = new Map()}

  // Отдельные мапы для хранения обработчиков конкретных компонентов
   __init2() {this.componentAddedHandlers = new Map()}
   __init3() {this.componentRemovedHandlers = new Map()}
   ...
   */
  on(event, handler) {
    if (!this.entityHandlers.has(event)) {
      this.entityHandlers.set(event, new Set());
    }
    this.entityHandlers.get(event).add(handler);
  }

  /**
   * Подписаться на добавление конкретного типа компонента
   * @param componentType Тип компонента (имя класса или строка-идентификатор)
   * @param handler Обработчик события
   */
  onComponentAdded(componentType, handler) {
    if (!this.componentAddedHandlers.has(componentType)) {
      this.componentAddedHandlers.set(componentType, new Set());
    }
    this.componentAddedHandlers.get(componentType).add(handler);
  }

  /**
   * Подписаться на удаление конкретного типа компонента
   * @param componentType Тип компонента (имя класса или строка-идентификатор)
   * @param handler Обработчик события
   */
  onComponentRemoved(componentType, handler) {
    if (!this.componentRemovedHandlers.has(componentType)) {
      this.componentRemovedHandlers.set(componentType, new Set());
    }
    this.componentRemovedHandlers.get(componentType).add(handler);
  }

  emitEntity(eventType, entity) {
    _optionalChain([this, 'access', _ => _.entityHandlers, 'access', _2 => _2.get, 'call', _3 => _3(eventType), 'optionalAccess', _4 => _4.forEach, 'call', _5 => _5((handler) => handler(entity))])
  }
  emitComponent(eventType, component, entity) {
    const handlerSets = eventType === 'added' ? this.componentAddedHandlers : this.componentRemovedHandlers

    const handlerSet = handlerSets.get(component)
    if (!handlerSet) return
    handlerSet.forEach((handler) => handler(entity[component], entity))
  }
}
...
const ECS = mod(() => {
  ...
  // type WorldWithQueries<E, Q> = World<E> & { queries: Q }
  /**
  ...
  class Bucket {
    /* VERSIONING */
     __init4() {this._version = 0}

     __init5() {this.ee = new ECSEventEmitter}
    ...
    constructor( _entities = []) {;this._entities = _entities;Bucket.prototype.__init4.call(this);Bucket.prototype.__init5.call(this);Bucket.prototype.__init6.call(this);
      const { bind } = utils
      bind(this, this.add, this.remove)
     ...
     */
     __init6() {this.entityPositions = new Map()}
    ...
    add(entity) {
      if (entity && !this.has(entity)) {
        ...
        this.ee.emitEntity('added', entity)
      }
    ...
    remove(entity) {
      ...
      this.ee.emitEntity('removed', entity)
  ...
  {
    constructor() {
      super();World.prototype.__init7.call(this);World.prototype.__init8.call(this);World.prototype.__init9.call(this);World.prototype.__init10.call(this);

      /* When entities are added, reindex them immediately */
      this.ee.on('added', (entity) => {
        entity['id'] = this.id(entity)
        this.reindex(entity)
        Object.keys(entity).forEach(((name) => {
          // @ts-ignore
          this.ee.emitComponent('added', name, entity)
        }))
      })

      /* When entities are removed, remove them from all known queries, and delete
      their IDs */
      this.ee.on('removed', (entity) => {
        this.queryList.forEach((query) => query.remove(entity))

        Object.keys(entity).forEach(((name) => {
          // @ts-ignore
          this.ee.emitComponent('removed', name, entity)
        }))
    ...
    static create(queryCb) {
      const world = new World()
      const queries = queryCb(world)
      return Object.assign(world, { queries })
    }
     ...
     */
    addComponent(entity, component, value = true ) {
      /* Return early if the entity already has the component. */
      if (entity[component] !== undefined) return value
      /* Set the component */
      entity[component] = value

      /* Trigger a reindexing */
      this.reindex(entity)
      this.ee.emitComponent('added', component, entity)
    ...
    removeComponent(entity, component) {
      ...
      this.ee.emitComponent('removed', component, entity)
    ...
    /* QUERIES */
     ...
     __init7() {this.queryList = new Set()}
    ...
    /* IDs */
     ...
     __init8() {this.entityToId = new Map()}
     __init9() {this.idToEntity = new Map()}
     __init10() {this.nextId = 0}
    ...
    onEntityAdded(cb) { this.ee.on('added', cb) }
    onEntityRemoved(cb) { this.ee.on('removed', cb) }
    onComponentAdded(component, cb) {
      this.ee.onComponentAdded(component, cb)
    }
    onComponentRemoved(component, cb) {
      this.ee.onComponentRemoved(component, cb)
    }
  }

  class Query extends Bucket {
     __init11() {this._isConnected = false}
    ...
    constructor( world,  config) {
      super();this.world = world;this.config = config;Query.prototype.__init11.call(this);Query.prototype.__init12.call(this);
      ...
      // this.onEntityAdded.onSubscribe.subscribe(EntityEvents.ADDED, () => this.connect())
      // this.onEntityRemoved.onSubscribe.subscribe(EntityEvents.REMOVED, () => this.connect())
      // this.eventEmitter.once("newListener", () => this.connect())
      // this.eventEmitter.once("newListener", () => console.log("newListener"))
    }
     ...
     */
    __init12() {this.want = mod(() => {
      return (entity) => {
        const withChecker = (component) => entity[component] !== undefined
        const withoutChecker = (component) => entity[component] === undefined
        const predicateChecker = (predicate) => predicate(entity)
        return (
          this.config.with.every(withChecker) &&
          this.config.without.every(withoutChecker) &&
          this.config.predicates.every(predicateChecker)
        )
      }
    })}
    ...
    entity(id) {
      ...
      if (entity === undefined || !this.has(entity)) return undefined
      return entity
    }
  }

  const normalizeComponents = (components) => [...new Set(components.sort().filter((c) => !!c && c !== ""))]

  const normalizePredicates = (predicates) => [...new Set(predicates)]
  ...
  const idGen = mod(() => {
    ...
    let nextId = 0
    return function idGen(obj) {
      const id = entityToId.get(obj)
      if (id !== undefined) return id
      entityToId.set(obj, nextId)
      return ++nextId
    }
  })

  function configKey(config) {
    return `${config.with.join(",")}:${config.without.join(",")}:${config.predicates.map((p) => idGen(p)).join(",")}`
  }
...
var Priority; (function (Priority) {
  const FIRST = 1000000; Priority[Priority["FIRST"] = FIRST] = "FIRST";
  const Highest = 10000; Priority[Priority["Highest"] = Highest] = "Highest";
  const High = 100; Priority[Priority["High"] = High] = "High";
  const Normal = 0; Priority[Priority["Normal"] = Normal] = "Normal";
  const Low = -100; Priority[Priority["Low"] = Low] = "Low";
  const Lowest = -10000; Priority[Priority["Lowest"] = Lowest] = "Lowest";
  const LAST = -1000000; Priority[Priority["LAST"] = LAST] = "LAST";
})(Priority || (Priority = {}));
...
var BoardState; (function (BoardState) {
  const Solved = 'Solved'; BoardState["Solved"] = Solved;
  const Unsolved = 'Unsolved'; BoardState["Unsolved"] = Unsolved;
  const Generating = 'Generating'; BoardState["Generating"] = Generating;
  const Empty = 'Empty'; BoardState["Empty"] = Empty;
...
var FlaskState; (function (FlaskState) {
  const Full = 'Full'; FlaskState["Full"] = Full;
  const Empty = 'Empty'; FlaskState["Empty"] = Empty;
  const Partial = 'Partial'; FlaskState["Partial"] = Partial;
  const Solved = 'Solved'; FlaskState["Solved"] = Solved;
  const Selected = 'Selected'; FlaskState["Selected"] = Selected;
...
class Flask {
  ...
  static updateState(flask) {
    ...
    if (stack.length < volume) {
      if (stack.length > 0) flask.state = FlaskState.Partial
      else flask.state = FlaskState.Empty
    } else {
      const example = stack[0]
      const isSolved = stack.find((ball) => ball !== example) === undefined
      flask.state = isSolved ? FlaskState.Solved : FlaskState.Full
    }
    return state !== flask.state
  }
  ...
  static isSelectable(flask) {
    return !(flask.state === FlaskState.Empty || flask.state === FlaskState.Solved)
  }
}

var LiquidType; (function (LiquidType) {
  const A = 'A'; LiquidType["A"] = A;
  const B = 'B'; LiquidType["B"] = B;
  const C = 'C'; LiquidType["C"] = C;
  const D = 'D'; LiquidType["D"] = D;
  const E = 'E'; LiquidType["E"] = E;
})(LiquidType || (LiquidType = {}));

var LevelMode; (function (LevelMode) {
  const CLASSIC_RANDOM = 'CLASSIC_RANDOM'; LevelMode["CLASSIC_RANDOM"] = CLASSIC_RANDOM;
})(LevelMode || (LevelMode = {}));
...
var EC; (function (EC) {
  ...
  const BoardSolved = 'BoardSolved'; EC["BoardSolved"] = BoardSolved;
...
const createEventWorld = () => {
  return ECS.World.create((w) => {
    const
      setBoard = w.with(EC.SetBoard),
      boardGenerated = w.with(EC.BoardGenerated),
      boardSolved = w.with(EC.BoardSolved),

      flaskClicked = w.with(EC.FlaskClicked, EC.ClickType),
      flaskSelected = w.with(EC.FlaskSelected),
      flaskDeselected = w.with(EC.FlaskDeselected),
      flaskSolved = w.with(EC.FlaskSolved),
      ...
      boardViewStateChanged = w.with(EC.BoardViewStateChanged),
      boardViewFadedOut = w.with(EC.BoardViewFadedOut)
...
var VC; (function (VC) {
  ...
  const Style = 'Style'; VC["Style"] = Style;
...
const DEFAULT_STYLE = {
  width: 128,
  height: 400,
  liquidPadding: { top: 20, bottom: 20, left: 20, right: 20 },
  scale: 1,
}

var BoardViewState; (function (BoardViewState) {
  const Start = 'Start'; BoardViewState["Start"] = Start;
  const Play = 'Play'; BoardViewState["Play"] = Play;
  const Restarting = 'Restarting'; BoardViewState["Restarting"] = Restarting;
...
const themes = {
    classic: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0x7A222E,
            [LiquidType.B]: 0xEDAE56,
            [LiquidType.C]: 0x629864,
            [LiquidType.D]: 0x1C4C70,
            [LiquidType.E]: 0x5C4B51,
        }
    },
    circle: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0x734D3F,
            [LiquidType.B]: 0xD9D2C5,
            [LiquidType.C]: 0x6C8C86,
            [LiquidType.D]: 0xA4A8BF,
            [LiquidType.E]: 0xA67968,
        }
    },
    cold: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0x81AFA5,
            [LiquidType.B]: 0x604E6C,
            [LiquidType.C]: 0x333C59,
            [LiquidType.D]: 0xC3CDE1,
            [LiquidType.E]: 0xE5E2EB,
        }
    },
    yantar: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0xECD199,
            [LiquidType.B]: 0xE79B6A,
            [LiquidType.C]: 0xD37255,
            [LiquidType.D]: 0xA8484C,
            [LiquidType.E]: 0x632240,
        }
    },
    norway: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0x616B77,
            [LiquidType.B]: 0xB8B7BD,
            [LiquidType.C]: 0xF3EEF1,
            [LiquidType.D]: 0xADC97F,
            [LiquidType.E]: 0x699139,
        }
    },
    pink: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0xF8CCC8,
            [LiquidType.B]: 0x4F5382,
            [LiquidType.C]: 0xA5D1D0,
            [LiquidType.D]: 0x97687C,
            [LiquidType.E]: 0x578470,
        }
    },
    vintage: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0x5C4B51,
            [LiquidType.B]: 0x8CBEB2,
            [LiquidType.C]: 0xF2EBBF,
            [LiquidType.D]: 0xF3B562,
            [LiquidType.E]: 0xF06060,
        }
    },
    warm_earth: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0xD49A54,
            [LiquidType.B]: 0xC45A4F,
            [LiquidType.C]: 0x8F9A4E,
            [LiquidType.D]: 0xE8D4B4,
            [LiquidType.E]: 0x7A4A33,
        }
    },
    sea: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0x4EB8A5,
            [LiquidType.B]: 0x8FC8E8,
            [LiquidType.C]: 0x336699,
            [LiquidType.D]: 0x7FC9C9,
            [LiquidType.E]: 0x2A5A6E,
        }
    },
    fruit_mix: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0xFF9933,
            [LiquidType.B]: 0xB3D954,
            [LiquidType.C]: 0xD9547A,
            [LiquidType.D]: 0xFFF07A,
            [LiquidType.E]: 0x6E4A7A,
        }
    },
    pastel_spring: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0xFFD1B3,
            [LiquidType.B]: 0xC9A8E8,
            [LiquidType.C]: 0xA8E8C9,
            [LiquidType.D]: 0xE8C9D1,
            [LiquidType.E]: 0xB3E8A8,
        }
    },
    grafite: {
        flaskBg: 0x261025,
        liquids: {
            [LiquidType.A]: 0x5A5A5A,
            [LiquidType.B]: 0x8C8C8C,
            [LiquidType.C]: 0xCCCCCC,
            [LiquidType.D]: 0x7A9999,
